...
// Currently Fusion is managing players. If there is no Fusion server available, the client starts one, but if there is a server available, the client connects directly to it without using Unity's Matchmaker.
